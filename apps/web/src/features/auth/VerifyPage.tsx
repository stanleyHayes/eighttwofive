    ...
    <Container component="main" maxWidth="lg">
      <Box sx={{ py: { xs: 8, md: 13 }, maxWidth: 480 }}>
        ...
        <Typography variant="h2" component="h1" sx={{ mt: 1.5, mb: 4 }}>
          {failed ? t("verify.headingFailed") : t("verify.headingPending")}
        </Typography>
        ...
        {failed ? (
          ...
          >
            ...
            <Typography sx={{ mt: 1.5 }}>
              {t("verify.errorBodyBefore")}{" "}
              <Link component={RouterLink} to="/login">
                {t("verify.errorLink")}
              </Link>
              {t("verify.errorBodyAfter")}
            </Typography>
          </Box>
        ) : (
          <Stack direction="row" spacing={2} sx={{ alignItems: "center" }}>
            <CircularProgress size={24} aria-label={t("verify.spinnerAria")} />
            <Typography variant="overline" sx={{ color: "text.secondary" }}>
              {t("verify.pendingStatus")}
            </Typography>
          </Stack>
        )}
      </Box>
